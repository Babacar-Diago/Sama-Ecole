<?php 
	...
	$req = $pdo->prepare('SELECT * FROM personnel WHERE identifiant = ?');
	...
	if ($user && $user['confirmation_token'] == $token) {
		...
		$req = $pdo->prepare("UPDATE personnel SET confirmation_token = NULL, confirmation_at = NOW() WHERE identifiant = ?"); 
		$req->execute([$user_id]);
